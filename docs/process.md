...
Two **projections** follow, both read-only. `docs/graph.mmd` is regenerated by
`pnpm run graph` (over GraphQL, because `gh issue list --json` exposes neither the issue type
nor the sub-issue edge) — not on demand by anybody, but by
`.github/workflows/graph.yml` on every issue event, which commits it straight to `main` over a
deploy key (ADR-1024). `pnpm run status` derives a Story's stage from the
...
│   ├── atlas-commands.html       # the six /atlas forms, and when each is right
...
`docs/graph.mmd` is deliberately *not* on this list. Closing those issues is what refreshes it,
unattended and within the minute — ADR-1024.
