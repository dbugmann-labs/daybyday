...
Issues for this repo live on `dbugmann-labs/daybyday`. Use the `gh` CLI for all operations.
...
`to-tickets` emits acceptance criteria into the issue body, and its `<issue-template>` is not
this repo's shape. It is **not** overridden and cannot be — the template is inline in the
plugin's own `SKILL.md`. Instead the skill is stopped after its breakdown quiz and never reaches
its publish step, so `orchestrator` writes the issues from `.github/ISSUE_TEMPLATE/story.yml`
(`docs/process.md` §10). If a ticket does arrive carrying an `## Acceptance criteria` block or a
`ready-for-agent` label, strip both and report it: the skill ran a step too far.
...
Traceability is one-way and write-once: the issue is created once via the API call below, the
branch name embeds the issue number and change id (`story/<issue#>-<change-id>`), and the PR
closes the issue. `docs/graph.mmd` is a read-only projection, regenerated and committed by
`.github/workflows/graph.yml` whenever an issue moves; nobody refreshes it by hand.
...
- **Attach a sub-issue**: there is no `gh` verb for this. It is an API call, and the field is
  the child's **database `id`**, not its issue number — passing the number silently attaches the
  wrong issue or fails:
...
- **Read an issue**: **not** `gh issue view <number> --comments`. In gh 2.83.0 that flag
  ...
  it prints *nothing at all* and exits 0 — an agent starting from it reads silence as a missing
  issue. Use two calls; the second printing nothing is correct:
...
- **Read an issue's type**: API only, as `.type.name`. gh 2.83.0 exposes no `--json` field for
  it on `gh issue view` or `gh issue list` — `--json issueType` is rejected as unknown — so the
  type the whole three-level model rests on is invisible to every `gh issue` subcommand:
...
- **List issues**: `gh issue list --state open --json number,title,body,labels,comments --jq '[.[] | {number, title, body, labels: [.labels[].name], comments: [.comments[].body]}]'`, with `--label` and `--state` filters. This returns no type; pair it with the call above when type matters.
...
It has to, because ADR-0012 dropped the project board and left open/closed as the primary state
signal. If `open` meant "work outstanding" on a Story and "this capability exists" on a Feature,
the one signal that survived would carry two meanings and stop answering *where is everything*.
...
close; closing that Feature makes its Epic ready. Check the rollup after each close:
...
`0/1` immediately after the child closed, correcting to `1/1` moments later. Fine for a glance,
wrong for a decision made in the same breath as the close. `docs/graph.mmd` draws a
ready-to-close parent with an amber border, derived from each issue's own state rather than the
rollup, so it does not lag.

**A triaged bug is not outstanding work for this purpose.** An issue carrying `needs-triage` and
no issue type sits outside the hierarchy by design — it is inbox, not plan. It does not hold its
capability's Feature open. When it is promoted to a Story, reopen the Feature then.
...
check 5 fails, and the honest fix is to put the new version to the human and record their answer
as a new comment — never to edit the old one, and never to record it yourself. Leaving the first
marker in place is deliberate: when a requirement changed, and who agreed to it, is worth more
than a tidy thread. ADR-1007.

Read it with the comments call under **Conventions**, not `gh issue view --comments`, which goes
silent on an issue that has none. `pnpm run check:g4` asserts it, and CI check 5 blocks the merge
if it is missing or signs a version that has moved on.

## Answers for skills that ask

- **"Publish to the issue tracker"** → create a GitHub issue, as a stub, per **What goes in an
  issue body** above.
- **"Fetch the relevant ticket"** → run the two-call read under **Conventions**, then open the
  change folder it links to. An issue read in isolation is not enough to implement from.
- **"PRs as a request surface"** → **no.** This repo does not triage external pull requests.
...
- **Wayfinding operations** → not applicable. `/wayfinder` is on the **Never invoke** list in
  `AGENTS.md`, so this repo keeps no wayfinder map, child tickets or `wayfinder:*` labels.
