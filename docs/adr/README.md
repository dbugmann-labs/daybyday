...
| [1034](1034-a-schedule-says-its-rhythm-in-words.md) | A schedule says its rhythm in the package's own words — "Mon, Wed, Sat", "Every 14 days", "The 25th", "3x a week" — and no payload accessor becomes public; extends ADR-1022 from a date to a rule; amended 2026-09-10 and 2026-09-11 with arguments that had lived only in requirement prose — a day-view row always says its rhythm, a start date is never said, seven times a week is not "Every day", and a weekday set of no day is "No day" rather than nothing; amended 2026-09-14 so a weekly quota said given a count says "1/3x a week", and a quota's day-screen row says and is its standing |
| [1035](1035-a-roster-never-lets-a-commitment-go.md) | A roster never lets a commitment go: removing is a third state beside kept and stopped, so every past day keeps its rows and day one stays unreachable without a marker; superseded 2026-09-23 by ADR-1060 |
...
| [1049](1049-a-change-writes-the-record-place-first.md) | A change to a commitment writes the record place before the roster place, and one that carries records leaves a save in progress beside the record place first, which whatever reads the places next undoes unless the roster holds what the records went to; amended 2026-09-14 so that replaces asking for the same change again as the repair, a torn save that cannot be undone withholds the places, and a record orphaned by any cause goes back only to its one possible source; and the kinds of refused change are counted in one requirement and named rather than numbered everywhere else, because withdrawing a kind renumbers every position after it and archived change folders are never edited |
| [1050](1050-a-week-is-monday-to-sunday-and-the-history-counts-it.md) | A week is Monday through the following Sunday on every phone, whatever its calendar setting, so a restored history reads the same wherever it lands; nothing happens when a week turns, since a miss is a record kind this product does not have and a count of misses is a streak in reverse; and the **history** answers a commitment's **standing** — the count of its week's days it was kept, through a date, uncapped, for any commitment and consulting no schedule — while the day screen is what says it. Discharges the three questions ADR-1015 deferred |
...
| [1052](1052-a-one-off-follows-today-until-it-is-done.md) | A one-off stands on exactly one day at a time and follows today while it is undone — the later of its date and today, then the day it was ticked, for good — so a past day draws only the one-offs ticked there; decided by the owner against the recommendation to leave it on its date as a commitment's miss stays in its day, because a thing owed once, left on a day that has gone, is invisible while still having to be done; the price is that a past day can draw differently on two different days, and the corollary that a one-off is never made done before its own date was taken at Stage 4 |
| [1054](1054-a-copy-is-the-values-and-it-knows-when.md) | A copy is the three stores' values as the app reads them — a torn save undone and every store read up into the form it writes now, never the files' bytes, which is what `take-out-an-unreadable-store` does instead — taken whole or refused with the store that would not read named; it carries a form of its own, independent of the three stores' forms, and the **moment** it was made, the first timestamp in the model, handed in at the edge as `today()` is and living in a copy and never in a store; and it leaves the phone as a file of DayByDay's own kind, `.daybyday`, so a later picker can offer copies alone |
| [1055](1055-a-look-back-reads-eras-by-resemblance.md) | A look-back originally worked out a commitment's earlier **eras** — the spans a rhythm change or an interval restart cut — by resemblance: name, kind and the day kept until being the day before the next day kept from, nearest first; the owner's call against the recommendation to stop at the boundary. Amended 2026-09-22, reversed by ADR-1059: an era now carries the commitment's own identity, so a look-back reads the chain off the roster instead of inferring it, and resemblance stands only as the one-time fold's own rule for reading a roster kept before a commitment had an identity |
| [1056](1056-a-restore-is-undone-as-it-was.md) | A restore is whole or nothing across the record, the roster and the one-offs: before it writes, it keeps what stood at those places and at the save-in-progress place in a restore in progress beside the record place. A refused write is put back at once, and a restore stopped partway is rolled back, never forward, the next time the places are opened, before a torn save is looked for. One that cannot be undone withholds all three places — bar a day screen returned to from a commitments screen that restored nothing, or from no screen at all, which is left exactly as it was — because a restore is said only once it is whole |
| [1057](1057-a-pages-layout-is-decided-at-the-grill.md) | A page's layout is decided at the grill, from a mockup: the last round of any Story grill that reaches the shell spawns `designer`, an Opus agent that writes one HTML file in scratch and nothing in the repo; the conductor publishes it as an artifact and the owner chooses; the wireframe lands in `grill.md` § *Layout* and `design.md` § *What the shell draws*, outside the 150, and the reviewer reads the walk against it. Decided after #272's page was first judged from its walk pictures at G7 and sent back, which cost three rounds |
| [1058](1058-a-copy-follows-a-kept-change.md) | A copy at the **copy place** follows a change the person kept and nothing else: every write either screen makes for a change the person made, one copy per change however many places it reached, and no copy at all for an open-time repair — a torn save or restore undone, an orphaned record carried back, the day-one take-on — because a repair puts back what the copy already mirrors. The change is kept first and answered exactly as it would be with no copy place, so a copy that cannot be made refuses nothing; the failure and the moment it first happened are held on the copy place and said on the commitments screen beside the last copy that succeeded, the next kept change retries on its own, and only an attempt ever finds out the folder is gone |
| [1059](1059-equality-is-the-identity-and-an-era-is-an-entry.md) | A commitment carries an **identity**, given once and never derived from or changed by its other parts; two commitments are the same commitment exactly when their identities are the same, however alike or unalike everything else. An **era** is a roster entry carrying that identity rather than a fourth part or a list the commitment owns, so a rename, a rhythm change and a kept-from move are all acts on one identity instead of a second commitment taking the first's place, and a look-back reads the chain off the roster rather than inferring it by resemblance. ADR-1023, ADR-1030, ADR-1035 and ADR-1055 are amended in place rather than rewritten |
| [1060](1060-a-deleted-commitment-takes-its-history-and-an-emptied-roster-stays-empty.md) | A commitment a person is done with is **deleted** — every era, its place and category, and every record against it — confirmed by its name typed back, and the removed state is retired, superseding ADR-1035; the record place is written first and put back if the roster refuses; a roster deletion leaves holding nothing is **emptied**, a mark its form and so every copy carries, so day one (ADR-1027) is never written over it; what the removed state still holds in a roster at the identity form is erased at the upgrade, and a roster at an earlier form is folded and erases nothing |
