...
| [1055](1055-a-look-back-reads-eras-by-resemblance.md) | A look-back works out a commitment's earlier **eras** — the spans a rhythm change or an interval restart cut — from what the roster already holds, by name, kind and the day kept until being the day before the next day kept from, nearest first; the owner's call against the recommendation to stop at the boundary. A recorded link is a fifth roster form with a migration and would still need this inference to read the rosters already on a phone, so the inference is the whole answer or it is not needed; the two readings it gets wrong — a same-day remove-and-redefine, and a kept-from correction that breaks a chain — were accepted with the decision |
