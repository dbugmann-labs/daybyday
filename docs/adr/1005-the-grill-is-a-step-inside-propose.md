...
**Grilling is the first step of Stage 4, not a stage.** `spec-author` grills and proposes in
one pass. The three obligations Stage 3 carried survive the move intact, and are now Stage 4's:

- `design.md` § **Open Questions** is filled in. "None." — with the reason — is a valid and
  required answer; an empty section is not one.
- New domain terms land in `CONTEXT.md`, which `spec-author` still writes alone.
- **The human still answers what the grill cannot settle.** That was never an agent's decision
  to make, and folding the stage does not make it one. It is stated in the *Who decides* column
  of Stage 4, in `docs/process.md` §4's obligation sentence, and in the `spec-author` agent
  definition.

**`grill-with-docs` stays enabled.** It is no longer named as a mandatory step, but it remains
the tool for the grilling step, it is what maintains `CONTEXT.md`, and `docs/process.md` §4
names it for the product-definition conversation upstream of Stage 0 — which this decision does
not touch. A skill you have stopped *requiring* is not a skill to disable; the kill list in
ADR-0009 is for skills that would do damage if invoked, and this is not one.
...
- **Nothing enforces that the grill happened**, and nothing did before either. `pnpm run status`
  ...
  the thinking. That is the same trade ADR-0010 makes for red-before-green: the artifact is
  checkable, the discipline is not, and saying so is better than implying otherwise.
