...
- `PRODUCT_BUNDLE_IDENTIFIER` is `com.dbugmann.daybyday` in both of the app target's build
  configurations, and the `simctl` commands in `docs/running-the-app.md` address the app by it. The
  `DayByDayUITests` bundle added by ADR-1029 takes `com.dbugmann.daybyday.uitests`; nothing keys a
  container to it, so it carries none of the weight above.
- **Signing is not committed to `project.pbxproj`, and this ADR does settle that much.** The project
  stays unsigned — `CODE_SIGN_IDENTITY = ""`, Xcode's *Do Not Code Sign* — so CI's simulator build
  needs no certificate and no profile on a runner. `scripts/install-on-phone.ts` passes the
  identity, `DEVELOPMENT_TEAM` and `CODE_SIGNING_ALLOWED=YES` on the command line, where they
  outrank the project. The shape is measured rather than preferred: a device build with the project
  as it stands **succeeds and is unsigned**, and setting only the identity in the project does not
  fix it, because `CODE_SIGNING_ALLOWED = NO` still suppresses the step (both checked 2026-09-06).
  The consequence to live with is that opening the project in the Xcode GUI to create a certificate
  rewrites `project.pbxproj` and drops the UI test target — that rewrite is thrown away, not
  committed. `docs/running-the-app.md` § *On your own phone*.
