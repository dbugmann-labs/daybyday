        ...
        """
        {
          ...
          "commitments": [
            ...
            },
            {
              "commitment": {
                "name": "Vitamins",
                "keptFrom": { "year": 2026, "month": 6, "day": 1 },
                "schedule": { "weekdays": ["monday", "wednesday"] }
              },
              "removed": false,
              "category": null
...
func aCommitmentDefinedOnEachOfTheFourRhythmsIsReadBackOnTheScheduleThatRhythmNames() throws {
    ...
    let rosterStore = try RosterStore(at: rosterPlace)
    #expect(rosterStore.roster.commitments.map(\.name) == ["Gym", "Finances", "Contact lenses", "Reading"])
    #expect(
        rosterStore.roster.commitments.map(\.rhythmInWords)
            == ["Mon, Wed, Sat", "The 25th", "Every 14 days", "3x a week"])
    #expect(rosterStore.roster.commitments.allSatisfy { $0.keptFrom == monday })
}
...
func aCommitmentOfEachOfTheFourKindsIsDefinedThroughACommitmentsScreenAndKeptWithThatKind() throws {
    ...
    let rosterStore = try RosterStore(at: rosterPlace)
    #expect(rosterStore.roster.commitments.map(\.name) == ["Gym", "Mood", "Journal", "Protein"])
    #expect(
        rosterStore.roster.commitments.map(\.kind)
            == [
                .tick, .number(range: Commitment.Range(lowest: 1, highest: 10)), .note,
                .total(target: Commitment.Target(120)!),
            ])
    #expect(rosterStore.roster.commitments.allSatisfy { $0.schedule == dailySchedule })
    #expect(rosterStore.roster.commitments.allSatisfy { $0.keptFrom == monday })
}
...
@MainActor
@Test("a commitments screen refuses a name that differs only in case or in blank space at its ends")
func aCommitmentsScreenRefusesANameThatDiffersOnlyInCaseOrInBlankSpaceAtItsEnds() throws {
    let rosterPlace = freshRosterPlace()
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    ...
    let lowercaseRefusal = screen.define(name: "gym", on: Rhythm(allWeekdays), keptFrom: monday, under: nil)
    let uppercaseRefusal = screen.define(name: "GYM", on: Rhythm(allWeekdays), keptFrom: monday, under: nil)
    let paddedRefusal = screen.define(name: " Gym ", on: Rhythm(allWeekdays), keptFrom: monday, under: nil)

    #expect(lowercaseRefusal == .nameAlreadyInUse("Gym"))
    #expect(uppercaseRefusal == .nameAlreadyInUse("Gym"))
    #expect(paddedRefusal == .nameAlreadyInUse("Gym"))
    #expect(screen.kept.map(\.name) == ["Gym"])
}
...
@MainActor
@Test("a commitments screen refuses a name a commitment its roster has stopped keeping has")
func aCommitmentsScreenRefusesANameACommitmentItsRosterHasStoppedKeepingHas() throws {
    let rosterPlace = freshRosterPlace()
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let allWeekdays: Schedule = .weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
...
func aCommitmentACommitmentsScreenRefusesForItsNameIsNotTakenOnASecondTime() throws {
    ...
    #expect(firstRefusal == .nameAlreadyInUse("Gym"))
    #expect(secondRefusal == .nameAlreadyInUse("Gym"))
...
@MainActor
@Test("a commitment defined through a commitments screen carries an identity of its own")
func aCommitmentDefinedThroughACommitmentsScreenCarriesAnIdentityOfItsOwn() throws {
    let rosterPlace = freshRosterPlace()
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    let rhythm: Rhythm = .weekdays([.monday, .wednesday, .saturday])

    let screen = CommitmentsScreen(asOf: monday, keepingRosterAt: rosterPlace)

    let firstRefusal = screen.define(name: "Gym", on: rhythm, keptFrom: monday, under: nil)
    #expect(firstRefusal == nil)

    let otherRoster = try RosterStore(at: rosterPlace)
    try otherRoster.rename(screen.kept.first!, to: "Lifting")
    screen.shown(asOf: monday)

    let secondRefusal = screen.define(name: "Gym", on: rhythm, keptFrom: monday, under: nil)
...
@MainActor
@Test("an earlier era of a commitment is in neither of a commitments screen's lists")
func anEarlierEraOfACommitmentIsInNeitherOfACommitmentsScreensLists() throws {
    let rosterPlace = freshRosterPlace()
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let gym = Commitment(
        name: "Gym", schedule: .weekdays([.monday, .wednesday, .saturday]), keptFrom: keptFrom)!
    ...
    let newEra = Commitment(
        era: gym, schedule: .weekdays([.tuesday, .thursday]), keptFrom: monday, kind: .tick)!

    let rosterStore = try RosterStore(at: rosterPlace)
    try rosterStore.add(gym)
    try rosterStore.put(era: newEra, on: gym, keptUntil: sunday, under: nil)
    ...
    try stoppedRosterStore.put(era: newEra, on: gym, keptUntil: sunday, under: nil)
    try stoppedRosterStore.retire(newEra, keptUntil: monday)

    let stoppedScreen = CommitmentsScreen(asOf: monday, keepingRosterAt: stoppedRosterPlace)

    #expect(stoppedScreen.stopped.map(\.rhythmInWords) == ["Tue, Thu"])
    #expect(stoppedScreen.kept.isEmpty)
}
...
@MainActor
@Test("a commitment stopped through a commitments screen on a day holding a record of it is kept until that day")
func aCommitmentStoppedThroughACommitmentsScreenOnADayHoldingARecordOfItIsKeptUntilThatDay() throws {
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let daily: Schedule = .weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
    ...
    try tickRecordStore.add(Tick(gym, on: monday)!)
    let tickScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: tickPlaces.roster, keepingRecordAt: tickPlaces.record)
    tickScreen.askToStopKeeping(gym)
    tickScreen.confirmStopKeeping()
    let laterTickStore = try RosterStore(at: tickPlaces.roster)
    #expect(laterTickStore.roster.commitments(on: monday).map(\.name) == ["Gym"])
    #expect(laterTickStore.roster.commitments(on: tuesday).isEmpty)
    ...
    let noteRecordStore = try RecordStore(at: notePlaces.record)
    try noteRecordStore.add(Note("Good day", for: journal, on: monday)!)
    let noteScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: notePlaces.roster, keepingRecordAt: notePlaces.record)
    noteScreen.askToStopKeeping(journal)
    noteScreen.confirmStopKeeping()
    let laterNoteStore = try RosterStore(at: notePlaces.roster)
    #expect(laterNoteStore.roster.commitments(on: monday).map(\.name) == ["Journal"])
    #expect(laterNoteStore.roster.commitments(on: tuesday).isEmpty)
    ...
    try numberRosterStore.add(mood)
    let numberRecordStore = try RecordStore(at: numberPlaces.record)
    try numberRecordStore.add(Number(5, for: mood, on: monday)!)
    let numberScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: numberPlaces.roster, keepingRecordAt: numberPlaces.record)
    numberScreen.askToStopKeeping(mood)
    numberScreen.confirmStopKeeping()
    let laterNumberStore = try RosterStore(at: numberPlaces.roster)
    #expect(laterNumberStore.roster.commitments(on: monday).map(\.name) == ["Mood"])
    #expect(laterNumberStore.roster.commitments(on: tuesday).isEmpty)

    let totalPlaces = freshRosterAndRecordPlaces()
    let proteinTarget = Commitment.Target(120)!
    let protein = Commitment(
        name: "Protein", schedule: daily, keptFrom: keptFrom, kind: .total(target: proteinTarget))!
    let totalRosterStore = try RosterStore(at: totalPlaces.roster)
    try totalRosterStore.add(protein)
    let totalRecordStore = try RecordStore(at: totalPlaces.record)
    try totalRecordStore.add(Addition(30, for: protein, on: monday)!)
    let totalScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: totalPlaces.roster, keepingRecordAt: totalPlaces.record)
    totalScreen.askToStopKeeping(protein)
    totalScreen.confirmStopKeeping()
    let laterTotalStore = try RosterStore(at: totalPlaces.roster)
    #expect(laterTotalStore.roster.commitments(on: monday).map(\.name) == ["Protein"])
    #expect(laterTotalStore.roster.commitments(on: tuesday).isEmpty)

    // A screen whose record place holds a run of bytes that is not a record stops as of the day
    // before instead, exactly as it ships when nothing is known to keep.
    let unreadablePlaces = freshRosterAndRecordPlaces()
    try FileManager.default.createDirectory(
        at: unreadablePlaces.roster.deletingLastPathComponent(), withIntermediateDirectories: true)
    let unreadableRosterStore = try RosterStore(at: unreadablePlaces.roster)
    try unreadableRosterStore.add(gym)
    try Data("not a record".utf8).write(to: unreadablePlaces.record)
    let unreadableScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: unreadablePlaces.roster,
        keepingRecordAt: unreadablePlaces.record)
    unreadableScreen.askToStopKeeping(gym)
    unreadableScreen.confirmStopKeeping()
    let laterUnreadableStore = try RosterStore(at: unreadablePlaces.roster)
    #expect(laterUnreadableStore.roster.commitments(on: sunday).map(\.name) == ["Gym"])
    #expect(laterUnreadableStore.roster.commitments(on: monday).isEmpty)
}

@MainActor
@Test("a record taken back on the day its commitment was stopped leaves the row and the day kept until as they were")
func aRecordTakenBackOnTheDayItsCommitmentWasStoppedLeavesTheRowAndTheDayKeptUntilAsTheyWere()
    throws
{
    ...
    commitmentsScreen.askToStopKeeping(gym)
    commitmentsScreen.confirmStopKeeping()

    let dayScreen = DayScreen(
        startingFrom: [], asOf: monday, keepingRecordAt: places.record,
        keepingRosterAt: places.roster)

    let rowBeforeTakeBack = try #require(dayScreen.dayView.rows.first { $0.name == "Gym" })
    #expect(rowBeforeTakeBack.isKept)

    try dayScreen.tick(rowBeforeTakeBack)

    let rowAfterTakeBack = try #require(dayScreen.dayView.rows.first { $0.name == "Gym" })
    #expect(!rowAfterTakeBack.isKept)

    let laterStore = try RosterStore(at: places.roster)
    #expect(laterStore.roster.commitments(on: monday).map(\.name) == ["Gym"])
    #expect(laterStore.roster.commitments(on: tuesday).isEmpty)
}

@MainActor
@Test("a commitment stopped through a commitments screen on the day its newest era began is stopped at the era before it")
func aCommitmentStoppedThroughACommitmentsScreenOnTheDayItsNewestEraBeganIsStoppedAtTheEraBeforeIt()
    throws
{
    let places = freshRosterAndRecordPlaces()
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let tueThu: Schedule = .weekdays([.tuesday, .thursday])
    let gym = Commitment(name: "Gym", schedule: tueThu, keptFrom: keptFrom)!
    let sunday = CalendarDate(year: 2026, month: 8, day: 30)!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    ...
    let refusal = screen.change(
        gym, toName: "Gym", on: .weekdays([.monday]), keptFrom: keptFrom, under: nil)
    #expect(refusal == nil)
    let changedGym = try #require(screen.kept.first)

    screen.askToStopKeeping(changedGym)
    screen.confirmStopKeeping()
    ...
    let laterStore = try RosterStore(at: places.roster)
    #expect(laterStore.roster.eras(of: laterStore.roster.stopped.first!).count == 1)
    #expect(laterStore.roster.keptFrom(of: laterStore.roster.stopped.first!) == keptFrom)
    #expect(laterStore.roster.commitments(on: sunday).map(\.name) == ["Gym"])
    #expect(laterStore.roster.commitments(on: monday).isEmpty)
}

@MainActor
@Test("a commitment stopped through a commitments screen on the day its newest era began keeps that era where the day holds a record of it")
func aCommitmentStoppedThroughACommitmentsScreenOnTheDayItsNewestEraBeganKeepsThatEraWhereTheDayHoldsARecordOfIt()
    throws
{
    let places = freshRosterAndRecordPlaces()
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    ...
    let recordStore = try RecordStore(at: places.record)
    try recordStore.add(Tick(changedGym, on: monday)!)
    screen.shown(asOf: monday)
    ...
    #expect(screen.stopped.map(\.name) == ["Gym"])
    #expect(screen.stopped.map(\.rhythmInWords) == ["Mon"])

    let laterStore = try RosterStore(at: places.roster)
    #expect(laterStore.roster.eras(of: laterStore.roster.stopped.first!).count == 2)
    #expect(laterStore.roster.commitments(on: monday).map(\.rhythmInWords) == ["Mon"])
    #expect(laterStore.roster.commitments(on: tuesday).isEmpty)
}

@MainActor
@Test("a commitment taken up again through a commitments screen days after its stop begins a new era on the day the screen was handed")
func aCommitmentTakenUpAgainThroughACommitmentsScreenDaysAfterItsStopBeginsANewEraOnTheDayTheScreenWasHanded()
    throws
{
    let rosterPlace = freshRosterPlace()
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    ...
    let rosterStore = try RosterStore(at: rosterPlace)
    try rosterStore.add(gym)
    try rosterStore.retire(gym, keptUntil: stoppedAsOf)

    let screen = CommitmentsScreen(asOf: monday, keepingRosterAt: rosterPlace)
    screen.keepAgain(gym)

    let laterStore = try RosterStore(at: rosterPlace)
    let eras = laterStore.roster.eras(of: laterStore.roster.commitments.first!)
    #expect(eras.count == 2)
    let newestEra = eras[0]
...
@MainActor
@Test("a commitment stopped and taken up again on one day through a commitments screen is one era, as though it had never been stopped")
func aCommitmentStoppedAndTakenUpAgainOnOneDayThroughACommitmentsScreenIsOneEraAsThoughItHadNeverBeenStopped()
    throws
{
    let places = freshRosterAndRecordPlaces()
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    ...
    screen.askToStopKeeping(gym)
    screen.confirmStopKeeping()
    screen.keepAgain(try #require(screen.stopped.first))

    #expect(screen.kept.map(\.name) == ["Gym"])
    #expect(screen.stopped.isEmpty)

    let laterStore = try RosterStore(at: places.roster)
    #expect(laterStore.roster.eras(of: laterStore.roster.commitments.first!).count == 1)
    #expect(laterStore.roster.commitments.first!.keptFrom == keptFrom)
    #expect(laterStore.roster.commitments(on: monday) == laterStore.roster.commitments)

    // A commitment stopped through one screen as of a day holding a record of it, and taken up
    // again through a different one opened the day after, reads back one era too.
    let tickPlaces = freshRosterAndRecordPlaces()
    let tickGym = Commitment(name: "Gym", schedule: daily, keptFrom: keptFrom)!
    let tuesday = CalendarDate(year: 2026, month: 9, day: 1)!
    let tickRosterStore = try RosterStore(at: tickPlaces.roster)
    try tickRosterStore.add(tickGym)
    let tickRecordStore = try RecordStore(at: tickPlaces.record)
    try tickRecordStore.add(Tick(tickGym, on: monday)!)

    let stopScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: tickPlaces.roster, keepingRecordAt: tickPlaces.record)
    stopScreen.askToStopKeeping(tickGym)
    stopScreen.confirmStopKeeping()

    let resumeScreen = CommitmentsScreen(
        asOf: tuesday, keepingRosterAt: tickPlaces.roster, keepingRecordAt: tickPlaces.record)
    resumeScreen.keepAgain(try #require(resumeScreen.stopped.first))

    let laterTickStore = try RosterStore(at: tickPlaces.roster)
    #expect(laterTickStore.roster.eras(of: laterTickStore.roster.commitments.first!).count == 1)
}

@MainActor
@Test("an interval commitment taken up again through a commitments screen begins its count on the day the screen was handed")
func anIntervalCommitmentTakenUpAgainThroughACommitmentsScreenBeginsItsCountOnTheDayTheScreenWasHanded()
    throws
{
    let rosterPlace = freshRosterPlace()
    let startDate = CalendarDate(year: 2026, month: 8, day: 4)!
    let nails = Commitment(
        name: "Nails", schedule: .everyNDays(DayInterval(days: 4)!, from: startDate),
        keptFrom: startDate)!
    let stoppedAsOf = CalendarDate(year: 2026, month: 8, day: 23)!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    ...
    try rosterStore.retire(nails, keptUntil: stoppedAsOf)

    let screen = CommitmentsScreen(asOf: monday, keepingRosterAt: rosterPlace)
    screen.keepAgain(nails)

    let laterStore = try RosterStore(at: rosterPlace)
    let newestEra = try #require(laterStore.roster.commitments(on: monday).first)
    #expect(newestEra.isDue(on: monday))
    #expect(newestEra.isDue(on: friday))
    #expect(!newestEra.isDue(on: tuesday))
}

@MainActor
@Test("a commitment defined and stopped on one day and taken up again on a later day is kept from that later day")
func aCommitmentDefinedAndStoppedOnOneDayAndTakenUpAgainOnALaterDayIsKeptFromThatLaterDay() throws {
    let places = freshRosterAndRecordPlaces()
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    let laterMonday = CalendarDate(year: 2026, month: 9, day: 7)!
    let daily: Rhythm = .weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
    ...
    let firstScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: places.roster, keepingRecordAt: places.record)
    let defineRefusal = firstScreen.define(name: "Gym", on: daily, keptFrom: monday, under: nil)
    #expect(defineRefusal == nil)

    firstScreen.askToStopKeeping(try #require(firstScreen.kept.first))
    firstScreen.confirmStopKeeping()

    let secondScreen = CommitmentsScreen(
        asOf: laterMonday, keepingRosterAt: places.roster, keepingRecordAt: places.record)
    secondScreen.keepAgain(try #require(secondScreen.stopped.first))

    #expect(secondScreen.kept.map(\.name) == ["Gym"])

    let laterStore = try RosterStore(at: places.roster)
    #expect(laterStore.roster.eras(of: laterStore.roster.commitments.first!).count == 1)
    #expect(laterStore.roster.commitments.first!.keptFrom == laterMonday)
}
...
@MainActor
@Test("taking a commitment up again is refused where a commitment the screen keeps already has its name")
func takingACommitmentUpAgainIsRefusedWhereACommitmentTheScreenKeepsAlreadyHasItsName() throws {
    let rosterPlace = freshRosterPlace()
    try FileManager.default.createDirectory(
        at: rosterPlace.deletingLastPathComponent(), withIntermediateDirectories: true)
    let bytes = Data(
    ...
    let screen = CommitmentsScreen(asOf: monday, keepingRosterAt: rosterPlace)
    let bytesAfterOpen = try Data(contentsOf: rosterPlace)
    let stoppedGym = screen.stopped.first!

    let refusal = screen.keepAgain(stoppedGym)
...
func aCommitmentDefinedUnderACategoryIsDrawnInThatCategorysGroupAndKeptUnderIt() throws {
    ...
    let later = try RosterStore(at: rosterPlace)
    #expect(later.roster.groups.map(\.category) == ["Supplements", nil])
    #expect(later.roster.groups.map { $0.commitments.map(\.name) } == [["Creatine"], ["Gym"]])
}
...
func aCommitmentDefinedUnderACategoryOfNothingButBlankSpaceIsUnderNoneAndIsNotRefused() throws {
    ...
    #expect(refusal == nil)
    #expect(screen.keptGroups.map(\.category) == [nil])
    #expect(screen.keptGroups.map { $0.commitments.map(\.name) } == [["Gym"]])
    ...
    #expect(secondRefusal == nil)
    #expect(screen.keptGroups.map(\.category) == [nil])
    #expect(screen.keptGroups.map { $0.commitments.map(\.name) } == [["Gym", "Journaling"]])
}
...
func aCategoryIsKeptExactlyAsItWasTypedOnTheForm() throws {
    ...
    #expect(screen.keptGroups.map(\.category) == [" Supplements ", "Supplements"])
    #expect(screen.keptGroups.map { $0.commitments.map(\.name) } == [["Creatine"], ["Magnesium"]])
}
...
@MainActor
@Test("a renamed commitment keeps every record already made, and the record place is not written")
func aRenamedCommitmentKeepsEveryRecordAlreadyMadeAndTheRecordPlaceIsNotWritten() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
...
@MainActor
@Test("a rename through a commitments screen reaches every era of the commitment")
func aRenameThroughACommitmentsScreenReachesEveryEraOfTheCommitment() throws {
    let places = freshRosterAndRecordPlaces()
    ...
    let secondRefusal = screen.change(
        gym, toName: "Lifting", on: .weekdays([.tuesday, .thursday]), keptFrom: keptFrom, under: nil)

    #expect(firstRefusal == nil)
    #expect(secondRefusal == nil)

    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.commitments(on: sunday).allSatisfy { $0.name == "Lifting" })
    #expect(laterRosterStore.roster.commitments(on: sunday).map(\.rhythmInWords).contains("Mon, Wed, Sat"))
    #expect(laterRosterStore.roster.commitments(on: monday).map(\.name) == ["Lifting"])
    #expect(laterRosterStore.roster.commitments(on: monday).map(\.rhythmInWords) == ["Tue, Thu"])
    #expect(screen.whatItIsMadeOf(gym)?.name == "Lifting")
    #expect(screen.whatItIsMadeOf(gym)?.keptFrom == keptFrom)
}

@MainActor
@Test("a commitment whose rhythm is changed through a commitments screen is given a new era from today")
func aCommitmentWhoseRhythmIsChangedThroughACommitmentsScreenIsGivenANewEraFromToday() throws {
    let places = freshRosterAndRecordPlaces()
...
@MainActor
@Test("a commitment whose range is changed through a commitments screen is given a new era from today")
func aCommitmentWhoseRangeIsChangedThroughACommitmentsScreenIsGivenANewEraFromToday() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.commitments(on: sunday).map(\.kind).contains(.number(range: originalRange)))
    #expect(
        laterRosterStore.roster.commitments(on: monday).map(\.kind)
            == [.number(range: Commitment.Range(lowest: 1, highest: 5))])
}

@MainActor
@Test("a target changed through a commitments screen puts a new era on and leaves every record standing")
func aTargetChangedThroughACommitmentsScreenPutsANewEraOnAndLeavesEveryRecordStanding()
    throws
{
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([
...
@MainActor
@Test("a range added to a number commitment carrying none, and one taken off, each put a new era on")
func aRangeAddedToANumberCommitmentCarryingNoneAndOneTakenOffEachPutANewEraOn() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
    ...
    #expect(
        commitmentsOnMonday.contains {
            $0.identity == weight.identity && $0.kind == .number(range: Commitment.Range(lowest: 40, highest: 150))
        })
    #expect(commitmentsOnMonday.contains { $0.identity == mood.identity && $0.kind == .number(range: nil) })

    let commitmentsOnSunday = laterRosterStore.roster.commitments(on: sunday)
    #expect(
        commitmentsOnSunday.contains { $0.identity == weight.identity && $0.kind == .number(range: nil) })
    #expect(
        commitmentsOnSunday.contains {
            $0.identity == mood.identity
                && $0.kind == .number(range: Commitment.Range(lowest: 1, highest: 10))
        })

    #expect(laterRosterStore.roster.eras(of: weight).count == 2)
    #expect(laterRosterStore.roster.eras(of: mood).count == 2)
    #expect(laterRosterStore.roster.keptFrom(of: weight) == keptFrom)
    #expect(laterRosterStore.roster.keptFrom(of: mood) == keptFrom)
}
...
@MainActor
@Test("a name and a rhythm changed in one save put the new name on every era")
func aNameAndARhythmChangedInOneSavePutTheNewNameOnEveryEra() throws {
    let places = freshRosterAndRecordPlaces()
    let originalSchedule = Schedule.weekdays([.monday, .wednesday, .saturday])
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let gym = Commitment(name: "Gym", schedule: originalSchedule, keptFrom: keptFrom)!
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.commitments(on: sunday).allSatisfy { $0.name == "Gym 🏋️" })
    #expect(
        laterRosterStore.roster.commitments(on: sunday).map(\.rhythmInWords).contains("Mon, Wed, Sat"))
    #expect(laterRosterStore.roster.commitments(on: monday).map(\.name) == ["Gym 🏋️"])
    #expect(laterRosterStore.roster.commitments(on: monday).map(\.rhythmInWords) == ["Tue, Thu"])
...
@MainActor
@Test("the day a commitment is kept from is moved onto a later era and the eras it leaves no day for are dropped")
func theDayACommitmentIsKeptFromIsMovedOntoALaterEraAndTheErasItLeavesNoDayForAreDropped() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
    ...
    let firstRefusal = screen.change(
        gym, toName: "Gym", on: .weekdays([.tuesday, .thursday]), keptFrom: june1st, under: nil)
    let secondRefusal = screen.change(
        gym, toName: "Gym", on: .weekdays([.tuesday, .thursday]), keptFrom: monday, under: nil)
    ...
    #expect(laterRosterStore.roster.keptFrom(of: gym) == monday)
    #expect(laterRosterStore.roster.commitments(on: monday).map(\.rhythmInWords) == ["Tue, Thu"])
    #expect(laterRosterStore.roster.commitments(on: sunday).first?.isDue(on: sunday) == false)
}

@MainActor
@Test("a name, an earlier day kept from and a rhythm changed in one save reach every era")
func aNameAnEarlierDayKeptFromAndARhythmChangedInOneSaveReachEveryEra() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([.monday, .wednesday, .saturday])
    let august1st = CalendarDate(year: 2026, month: 8, day: 1)!
    let june1st = CalendarDate(year: 2026, month: 6, day: 1)!
    let gym = Commitment(name: "Gym", schedule: schedule, keptFrom: august1st)!
    let sunday = CalendarDate(year: 2026, month: 8, day: 30)!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    ...
    let refusal = screen.change(
        gym, toName: "Gym 🏋️", on: .weekdays([.tuesday, .thursday]), keptFrom: june1st, under: nil)

    #expect(refusal == nil)

    let laterRosterStore = try RosterStore(at: places.roster)
    let onSunday = laterRosterStore.roster.commitments(on: sunday)
    #expect(onSunday.allSatisfy { $0.name == "Gym 🏋️" })
    #expect(onSunday.map(\.rhythmInWords).contains("Mon, Wed, Sat"))
    #expect(onSunday.map(\.keptFrom).contains(june1st))
    let onMonday = laterRosterStore.roster.commitments(on: monday)
    #expect(onMonday.map(\.name) == ["Gym 🏋️"])
    #expect(onMonday.map(\.rhythmInWords) == ["Tue, Thu"])
    #expect(onMonday.map(\.keptFrom) == [monday])

    #expect(screen.kept.count == 1)
    #expect(screen.kept.first?.name == "Gym 🏋️")
    #expect(screen.kept.first?.rhythmInWords == "Tue, Thu")
    #expect(screen.whatItIsMadeOf(gym)?.keptFrom == june1st)
}

@MainActor
@Test("a change writes nothing at the record place, whatever it changes")
func aChangeWritesNothingAtTheRecordPlaceWhateverItChanges() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
    ...
    let secondRefusal = screen.change(
        gym, toName: "Gym 🏋️", on: Rhythm(schedule), keptFrom: june1st, under: nil)
    let thirdRefusal = screen.change(
        gym, toName: "Gym 🏋️", on: .weekdays([.tuesday, .thursday]), keptFrom: june1st, under: nil)

    #expect(firstRefusal == nil)
    #expect(secondRefusal == nil)
    #expect(thirdRefusal == nil)
    #expect(try Data(contentsOf: places.record) == recordBytes)
}

@MainActor
@Test("a commitments screen says a commitment's earliest era's day kept from and its newest era's rhythm")
func aCommitmentsScreenSaysACommitmentsEarliestErasDayKeptFromAndItsNewestErasRhythm() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([.monday, .wednesday, .saturday])
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let gym = Commitment(name: "Gym", schedule: schedule, keptFrom: keptFrom)!
    ...
    #expect(refusal == nil)
    #expect(screen.whatItIsMadeOf(gym)?.keptFrom == keptFrom)
    #expect(screen.whatItIsMadeOf(gym)?.rhythm == .weekdays([.tuesday, .thursday]))
}
...
@MainActor
@Test("a change to a name another commitment already has is refused, kept or stopped alike")
func aChangeToANameAnotherCommitmentAlreadyHasIsRefusedKeptOrStoppedAlike() throws {
    let schedule = Schedule.weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    ...
    #expect(keptRefusal == .nameAlreadyInUse("Run"))
...
func aStoppedCommitmentRenamedThroughACommitmentsScreenStaysStoppedOnTheDayItWasKeptUntil() throws {
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.commitments(on: sunday) == [gym, journaling])
...
@MainActor
@Test("a rhythm changed on the first date the calendar supports puts the new era on as of that day itself")
func aRhythmChangedOnTheFirstDateTheCalendarSupportsPutsTheNewEraOnAsOfThatDayItself() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([.monday, .wednesday, .saturday])
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.eras(of: gym).count == 1)
    #expect(laterRosterStore.roster.keptFrom(of: gym) == firstSupportedDate)
    #expect(
        laterRosterStore.roster.commitments(on: firstSupportedDate).map(\.rhythmInWords)
            == ["Tue, Thu"])
    #expect(
        laterRosterStore.roster.commitments(on: secondSupportedDate).map(\.rhythmInWords)
            == ["Tue, Thu"])
}

@MainActor
@Test("a rhythm changed and changed back on one day leaves the commitment as it was that morning")
func aRhythmChangedAndChangedBackOnOneDayLeavesTheCommitmentAsItWasThatMorning() throws {
    let places = freshRosterAndRecordPlaces()
    let originalSchedule = Schedule.weekdays([.monday, .wednesday, .saturday])
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let gym = Commitment(name: "Gym", schedule: originalSchedule, keptFrom: keptFrom)!
    let mood = Commitment(
        name: "Mood", schedule: originalSchedule, keptFrom: keptFrom,
        kind: .number(range: Commitment.Range(lowest: 1, highest: 10)))!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    ...
    let awayRefusal = screen.change(
        gym, toName: "Gym", on: .weekdays([.tuesday, .thursday]), keptFrom: keptFrom, under: nil)
    let backRefusal = screen.change(gym, toName: "Gym", on: Rhythm(originalSchedule), keptFrom: keptFrom, under: nil)
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.eras(of: gym).count == 1)
    #expect(laterRosterStore.roster.commitments.map(\.rhythmInWords).contains("Mon, Wed, Sat"))
    #expect(laterRosterStore.roster.keptFrom(of: gym) == keptFrom)
    #expect(
        laterRosterStore.roster.commitments(on: CalendarDate(year: 2026, month: 8, day: 31)!)
            .filter { $0.identity == gym.identity }.map(\.rhythmInWords) == ["Mon, Wed, Sat"])

    let moodAwayRefusal = screen.change(
        mood, toName: "Mood", on: Rhythm(originalSchedule), keptFrom: keptFrom, under: nil, lowest: "1",
        highest: "5")
    let moodBackRefusal = screen.change(
        mood, toName: "Mood", on: Rhythm(originalSchedule), keptFrom: keptFrom, under: nil, lowest: "1",
        highest: "10")

    #expect(moodAwayRefusal == nil)
    #expect(moodBackRefusal == nil)

    let finalRosterStore = try RosterStore(at: places.roster)
    #expect(finalRosterStore.roster.eras(of: mood).count == 1)
    #expect(
        finalRosterStore.roster.commitments.first { $0.identity == mood.identity }?.kind
            == .number(range: Commitment.Range(lowest: 1, highest: 10)))
    #expect(finalRosterStore.roster.keptFrom(of: mood) == keptFrom)
}

@MainActor
@Test("a rhythm changed three times on one day leaves one era for that day and a roster that can be read")
func aRhythmChangedThreeTimesOnOneDayLeavesOneEraForThatDayAndARosterThatCanBeRead() throws {
    let places = freshRosterAndRecordPlaces()
    let originalSchedule = Schedule.weekdays([.monday, .wednesday, .saturday])
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let gym = Commitment(name: "Gym", schedule: originalSchedule, keptFrom: keptFrom)!
    ...
    let firstRefusal = screen.change(
        gym, toName: "Gym", on: .weekdays([.tuesday, .thursday]), keptFrom: keptFrom, under: nil)
    let secondRefusal = screen.change(
        gym, toName: "Gym", on: Rhythm(originalSchedule), keptFrom: keptFrom, under: nil)
    let thirdRefusal = screen.change(
        gym, toName: "Gym", on: .weekdays([.tuesday, .thursday]), keptFrom: keptFrom, under: nil)
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.eras(of: gym).map(\.rhythmInWords) == ["Tue, Thu", "Mon, Wed, Sat"])
    #expect(
        laterRosterStore.roster.eras(of: gym).first?.keptFrom
            == CalendarDate(year: 2026, month: 8, day: 31)!)
    #expect(
        laterRosterStore.roster.eras(of: gym).last?.keptFrom == keptFrom)

    let shownAgainScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: places.roster, keepingRecordAt: places.record)
    #expect(shownAgainScreen.kept.map(\.name) == ["Gym"])
    #expect(shownAgainScreen.kept.first?.rhythmInWords == "Tue, Thu")
}

@MainActor
@Test("a commitment defined and changed on one day keeps one era, kept from that day")
func aCommitmentDefinedAndChangedOnOneDayKeepsOneEraKeptFromThatDay() throws {
    let places = freshRosterAndRecordPlaces()
    let originalSchedule = Schedule.weekdays([.monday, .wednesday, .saturday])
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    ...
    let defineRefusal = screen.define(
        name: "Gym", on: Rhythm(originalSchedule), keptFrom: monday, under: nil)
    guard let gym = screen.kept.first else {
        Issue.record("expected \"Gym\" to be taken on")
        return
    }
    let awayRefusal = screen.change(
        gym, toName: "Gym", on: .weekdays([.tuesday, .thursday]), keptFrom: monday, under: nil)
    let backRefusal = screen.change(gym, toName: "Gym", on: Rhythm(originalSchedule), keptFrom: monday, under: nil)
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.eras(of: gym).count == 1)
    #expect(laterRosterStore.roster.commitments.map(\.rhythmInWords) == ["Mon, Wed, Sat"])
    #expect(laterRosterStore.roster.keptFrom(of: gym) == monday)
}

@MainActor
@Test("an interval rhythm changed away and back on one day keeps the start date it had")
func anIntervalRhythmChangedAwayAndBackOnOneDayKeepsTheStartDateItHad() throws {
    let places = freshRosterAndRecordPlaces()
    let august4th = CalendarDate(year: 2026, month: 8, day: 4)!
    ...
    let restored = laterRosterStore.roster.commitments.first!
    #expect(restored.rhythmInWords == "Every 4 days")
    #expect(laterRosterStore.roster.keptFrom(of: nails) == august4th)
    #expect(restored.isDue(on: august30th))
    #expect(restored.isDue(on: september3rd))
    #expect(!restored.isDue(on: monday))
}
...
func aChangeBackOnOneDayIsRefusedWhereARecordMadeThatDayWouldBeLeftNotDue() throws {
    let places = freshRosterAndRecordPlaces()
    let originalSchedule = Schedule.weekdays([.tuesday, .thursday])
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let gym = Commitment(name: "Gym", schedule: originalSchedule, keptFrom: keptFrom)!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    ...
    let changedGym = screen.kept.first!
    ...
    #expect(backRefusal == .wouldLeaveARecordedDayNotDue)

    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.commitments(on: monday).map(\.rhythmInWords) == ["Mon"])
}

@MainActor
@Test("a restart reaches behind a change made days ago and replaces it")
func aRestartReachesBehindAChangeMadeDaysAgoAndReplacesIt() throws {
    let places = freshRosterAndRecordPlaces()
    let august4th = CalendarDate(year: 2026, month: 8, day: 4)!
    let august6th = CalendarDate(year: 2026, month: 8, day: 6)!
    let august17th = CalendarDate(year: 2026, month: 8, day: 17)!
    let august18th = CalendarDate(year: 2026, month: 8, day: 18)!
    let august20th = CalendarDate(year: 2026, month: 8, day: 20)!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    let nails = Commitment(
        name: "Nails", schedule: .everyNDays(DayInterval(days: 4)!, from: august6th),
        keptFrom: august4th)!
    ...
    let changeScreen = CommitmentsScreen(
        asOf: august20th, keepingRosterAt: places.roster, keepingRecordAt: places.record)
    let changeRefusal = changeScreen.change(
        nails, toName: "Nails", on: .everyNDays(5), keptFrom: august4th, under: nil)
    #expect(changeRefusal == nil)
    ...
    #expect(restartRefusal == nil)

    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.eras(of: nails).count == 2)
    let newer = laterRosterStore.roster.eras(of: nails)[0]
    let older = laterRosterStore.roster.eras(of: nails)[1]
    #expect(newer.rhythmInWords == "Every 5 days")
    #expect(newer.keptFrom == august18th)
    #expect(older.rhythmInWords == "Every 4 days")
    #expect(older.keptFrom == august4th)
    #expect(laterRosterStore.roster.commitments(on: august17th).count == 2)
    #expect(laterRosterStore.roster.commitments(on: august18th).count == 1)
    ...
    let firstRestartRefusal = firstRestartScreen.restart(
        alike, from: CalendarDate(year: 2026, month: 8, day: 25)!)
    #expect(firstRestartRefusal == nil)

    let secondRestartScreen = CommitmentsScreen(
        asOf: monday, keepingRosterAt: places.roster, keepingRecordAt: places.record)
    let currentAlike = secondRestartScreen.kept.first { $0.identity == alike.identity }!
    let secondRestartRefusal = secondRestartScreen.restart(
        currentAlike, from: CalendarDate(year: 2026, month: 8, day: 23)!)
    #expect(secondRestartRefusal == nil)

    let finalRosterStore = try RosterStore(at: places.roster)
    #expect(finalRosterStore.roster.eras(of: alike).count == 2)
    #expect(
        finalRosterStore.roster.eras(of: alike).first?.rhythmInWords == "Every 4 days")
    #expect(
        finalRosterStore.roster.eras(of: alike).first?.keptFrom
            == CalendarDate(year: 2026, month: 8, day: 23)!)
}

@MainActor
@Test("a commitment kept from a day after today, changed before that day, keeps the day it is kept from")
func aCommitmentKeptFromADayAfterTodayChangedBeforeThatDayKeepsTheDayItIsKeptFrom() throws {
    let places = freshRosterAndRecordPlaces()
    let originalSchedule = Schedule.weekdays([.monday, .wednesday, .saturday])
    let september4th = CalendarDate(year: 2026, month: 9, day: 4)!
    let september1st = CalendarDate(year: 2026, month: 9, day: 1)!
    let gym = Commitment(name: "Gym", schedule: originalSchedule, keptFrom: september4th)!
    let nails = Commitment(
        name: "Nails", schedule: .everyNDays(DayInterval(days: 4)!, from: september4th),
        keptFrom: september4th)!
    ...
    let screen = CommitmentsScreen(
        asOf: september1st, keepingRosterAt: places.roster, keepingRecordAt: places.record)

    let refusal = screen.change(
        gym, toName: "Gym", on: .weekdays([.tuesday, .thursday]), keptFrom: september4th, under: nil)
    let nailsRefusal = screen.change(nails, toName: "Nails", on: .everyNDays(5), keptFrom: september4th, under: nil)

    #expect(refusal == nil)
    #expect(nailsRefusal == nil)
...
@MainActor
@Test("a restart reaching behind a change is refused where a record the change's era holds would be left not due")
func aRestartReachingBehindAChangeIsRefusedWhereARecordTheChangesEraHoldsWouldBeLeftNotDue() throws {
    let places = freshRosterAndRecordPlaces()
    let august4th = CalendarDate(year: 2026, month: 8, day: 4)!
    let august6th = CalendarDate(year: 2026, month: 8, day: 6)!
    let august18th = CalendarDate(year: 2026, month: 8, day: 18)!
    let august20th = CalendarDate(year: 2026, month: 8, day: 20)!
    let august25th = CalendarDate(year: 2026, month: 8, day: 25)!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    let nails = Commitment(
        name: "Nails", schedule: .everyNDays(DayInterval(days: 4)!, from: august6th),
        keptFrom: august4th)!
    ...
    let changeRefusal = changeScreen.change(
        nails, toName: "Nails", on: .everyNDays(5), keptFrom: august4th, under: nil)
    #expect(changeRefusal == nil)
    let changedNails = changeScreen.kept.first!

    let recordStore = try RecordStore(at: places.record)
    try recordStore.add(Tick(changedNails, on: august25th)!)
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    #expect(laterRosterStore.roster.eras(of: nails).count == 2)
    #expect(laterRosterStore.roster.eras(of: nails).first?.rhythmInWords == "Every 5 days")
}
...
func aCommitmentsScreenHoldsARefusedChangeAgainstTheCommitmentItWasAskedToChange() throws {
    ...
    #expect(refusal == .nameAlreadyInUse("Run"))
    #expect(screen.refusedChange == .changing(gym, .nameAlreadyInUse("Run")))
}
...
func whatACommitmentsScreenHoldsAboutARefusedChangeEndsWhenAChangeToACommitmentIsKept() throws {
    ...
    #expect(firstRefusal == .nameAlreadyInUse("Run"))
    #expect(screen.refusedChange == .changing(gym, .nameAlreadyInUse("Run")))
...
func aChangeOfRhythmThroughACommitmentsScreenPutsTheCommitmentUnderTheCategoryItWasGiven() throws {
    ...
    #expect(refusal == nil)
    #expect(screen.keptGroups == [Roster.Group(category: "Morning", commitments: [gym])])
    #expect(screen.kept.map(\.rhythmInWords) == ["Tue, Thu"])
}
...
@MainActor
@Test("a name and a rhythm changed in one save onto a name another commitment has are refused")
func aNameAndARhythmChangedInOneSaveOntoANameAnotherCommitmentHasAreRefused()
    throws
{
    let places = freshRosterAndRecordPlaces()
    let originalSchedule = Schedule.weekdays([.monday, .wednesday, .saturday])
...
@MainActor
@Test("a change naming the name the commitment already has is not refused for it")
func aChangeNamingTheNameTheCommitmentAlreadyHasIsNotRefusedForIt() throws {
    let places = freshRosterAndRecordPlaces()
    let schedule = Schedule.weekdays([
        .monday, .tuesday, .wednesday, .thursday, .friday, .saturday, .sunday,
    ])
...
@MainActor
@Test(
    "an interval commitment whose start differs from the day it is kept from is renamed on a new rhythm and the era it gives way to keeps its start"
)
func anIntervalCommitmentWhoseStartDiffersFromTheDayItIsKeptFromIsRenamedOnANewRhythmAndTheEraItGivesWayToKeepsItsStart()
    throws
{
    let places = freshRosterAndRecordPlaces()
    let august6th = CalendarDate(year: 2026, month: 8, day: 6)!
    ...
    let laterRosterStore = try RosterStore(at: places.roster)
    let onAugust30th = laterRosterStore.roster.commitments(on: august30th)
    #expect(onAugust30th.map(\.name) == ["Nails 💅", "Nails 💅"])
    #expect(onAugust30th.map(\.rhythmInWords) == ["Sun", "Every 4 days"])
    #expect(onAugust30th.last?.keptFrom == august4th)
...
@MainActor
@Test("restarting an interval commitment moves no record and writes nothing at the record place")
func restartingAnIntervalCommitmentMovesNoRecordAndWritesNothingAtTheRecordPlace() throws {
    let places = freshRosterAndRecordPlaces()
    let august1st = CalendarDate(year: 2026, month: 8, day: 1)!
    ...
    #expect(screen.whatItIsMadeOf(nails)?.keptFrom == august1st)
}
...
func anIntervalCommitmentRestartedFromTheDayItIsKeptFromIsKeptOnNoDateBeforeTheRestart() throws {
    ...
    #expect(rosterStoreAfterwards.roster.commitments(on: august4th) == [nails])
}
...
@MainActor
@Test("a restart is refused for no name and for no record already kept")
func aRestartIsRefusedForNoNameAndForNoRecordAlreadyKept() throws {
    let places = freshRosterAndRecordPlaces()
    let august4th = CalendarDate(year: 2026, month: 8, day: 4)!
    let august6th = CalendarDate(year: 2026, month: 8, day: 6)!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
    let nails = Commitment(
        name: "Nails", schedule: .everyNDays(DayInterval(days: 4)!, from: august6th),
        keptFrom: august4th)!
    let nails2 = Commitment(
        name: "Nails 2", schedule: .everyNDays(DayInterval(days: 4)!, from: monday), keptFrom: monday)!

    let rosterStore = try RosterStore(at: places.roster)
    try rosterStore.add(nails)
    try rosterStore.add(nails2)
    let recordStore = try RecordStore(at: places.record)
    try recordStore.add(Tick(nails, on: august6th)!)
    ...
    #expect(refusal == nil)
    #expect(screen.kept.map(\.name) == ["Nails", "Nails 2"])

    let recordStoreAfterwards = try RecordStore(at: places.record)
    #expect(recordStoreAfterwards.history.isKept(nails, on: august6th))
    #expect(screen.whatItIsMadeOf(nails)?.keptFrom == august4th)
}
...
@MainActor
@Test("a record of an era a roster holds is not an orphan")
func aRecordOfAnEraARosterHoldsIsNotAnOrphan() throws {
    let places = freshRosterAndRecordPlaces()
    let keptFrom = CalendarDate(year: 2026, month: 1, day: 1)!
    let august3rd = CalendarDate(year: 2026, month: 8, day: 3)!
    let monday = CalendarDate(year: 2026, month: 8, day: 31)!
...
@MainActor
@Test("a restart refused for a day already recorded on is about the restart day field")
func aRestartRefusedForADayAlreadyRecordedOnIsAboutTheRestartDayField() throws {
    let places = freshRosterAndRecordPlaces()
    let august4th = CalendarDate(year: 2026, month: 8, day: 4)!
    let august6th = CalendarDate(year: 2026, month: 8, day: 6)!
    let august29th = CalendarDate(year: 2026, month: 8, day: 29)!
