...
public final class CopyPlace {
    ...
    /// Opens the birthday ticks at `place`, telling `.laterForm` apart from every other reason
    /// `BirthdayStore` can refuse to open — mirrors `DayScreen`'s own `openBirthdays(at:)`.
    private static func openBirthdayTicksTellingLaterVersionApart(
        at place: URL
    ) -> (store: BirthdayStore?, cause: CommitmentsScreen.StoreNotRead.Cause?) {
        do {
            return (try BirthdayStore(at: place), nil)
    ...
    /// place both stand beside; the birthday ticks are read on their own merits regardless —
    ...
    static func readStores(
        ...
        guard
            ...
            var notRead: [CommitmentsScreen.StoreNotRead] = [.record, .roster, .oneOffs].map {
