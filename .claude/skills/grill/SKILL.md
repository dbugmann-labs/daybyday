...
1. **The questions and their answers are written to a file**, because a conversation is not a
   ...
   where `None.` is valid and required, with the reason. Where the Story reaches
   `src/DayByDay/` it also carries `## Layout`: the option the human chose in the layout round
   from `designer`'s mockup, the artifact URL and that option's wireframe (ADR-1057). At Stage 1 there is no change folder yet and the
