...
`mattpocock-skills:grilling` prescribes, and **this repository follows that protocol as written —
the whole frontier in one round, and no departure from it.** What is local is only the medium: a
round is *asked* with `AskUserQuestion` rather than printed as markdown, so it can be answered
from the keyboard. Compose each question the way `grilling` describes — a title, a body, and the
answer you would give — and never wrap a round in a five-part block:
...
The markdown above is how you *compose* a round. `AskUserQuestion` is how you *ask* it. Printing
the block as well is allowed — for a question whose body is too long to sit in a picker, it is
the right call — but it is never required, and it is not a substitute for asking.

- **The whole frontier goes in one round.** The tool takes four questions at a time, so a
  six-wide frontier is two calls back to back — still one round, and the frontier is **not**
  recomputed until every question in it has been answered. **The cap must never shrink a round:**
  four asked and two dropped is the exact failure this shape was written to stop, and it is the
  one that now looks like correct behaviour from where the human sits.
...
- **A question you cannot name two answers for is one you have not thought about.** That is the
  ...
  not ready to ask.
- **When a round spans two calls, say so** — *3 of 6* in the question body — so the human knows
  the round has not ended and can see how much of it is left.
...
**Recompute the frontier after every round**, and keep going until it is empty. Then print a `▸`
