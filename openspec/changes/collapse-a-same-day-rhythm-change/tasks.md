...
- [ ] 1.3 *A look-back chains every era of the commitment it was asked about* puts three eras alike in
...
- [ ] 1.4 Sweep every carried test that puts an era on, restarts, or writes a roster by hand for one
...
- [ ] 2.1 an era put on as of a day before the day the era it gives way to is kept from drops that era — catches ending it and keeping it
- [ ] 2.2 an era put on as of the first supported date and one as of the last are both accepted — catches an overlap left where the date is after the new era's day kept from
- [ ] 2.3 an era put on as of a day before a later era began replaces every era begun after that day — catches ending only the newest
- [ ] 2.4 an era put on alike the one it would give way to leaves that one the newest, as it was — catches joining only across a dropped era

## 3. The reader — mended on every read

- [ ] 3.1 a stored roster holding eras that hold no day is read back without them — catches the duplicate guard still running before the mend
- [ ] 3.2 alike eras standing side by side are read back as one — catches joining under the newer's day kept from
- [ ] 3.3 eras holding the same days are read back with the newer keeping them — catches an older era carrying no day kept until left open
- [ ] 3.4 the newest era of a commitment stopped on the day it began is read back as it is — catches the mend deciding #306
- [ ] 3.5 a roster kept before a commitment had an identity holding an era that holds no day is read back without it — catches `folded()` left unmended
- [ ] 3.6 a copy holding eras that hold no day is restored with them mended — catches `CopyDocument` reading around the mend
- [ ] 3.7 a roster store holding what could not be a roster is refused — the carried test with its one-era-twice fixture gone, still green

## 4. `CommitmentsScreen` — change and restart

- [ ] 4.1 a rhythm changed and changed back on one day leaves the commitment as it was that morning — catches a new era alike the old one left standing
- [ ] 4.2 a rhythm changed three times on one day leaves one era for that day and a roster that can be read — catches the lock-out itself
- [ ] 4.3 a commitment defined and changed on one day keeps one era, kept from that day
- [ ] 4.4 an interval rhythm changed away and back on one day keeps the start date it had — catches the change back restarting the count
- [ ] 4.5 a change back on one day is refused where a record made that day would be left not due — catches the refusal skipped for a same-day change
...
- [ ] 5.1 `restart`'s special case for a restart on the day kept from goes, the put now covering it;
...
- [ ] 6.1 `pnpm run verify` green, and `swift test` from `src/DayByDayKit` reporting every test
  passing, the count read off the run.
- [ ] 6.2 `openspec validate collapse-a-same-day-rhythm-change --strict` exits 0 and `pnpm run checks`
