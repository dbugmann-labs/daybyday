...
nothing, or from no screen at all: that screen keeps what it had already read, opens nothing and is
left exactly as it was, which is what `day-screen` asks of the state that stands across being
returned to. It rolls back, as a torn save does (ADR-1049); `stoppingAfter` is the test seam that
stops it partway, as `SaveInProgress.keep` is for a torn save.
