...
- [x] 2.1 *A commitment is a name, a schedule, and the day it is kept from* — 39% of its prose is
      cross-reference and restatement; the rules it defers to are stated where they live
- [x] 2.2 *A commitment's name says something* — blank is ADR-1039's one test, cited not restated
- [x] 2.3 *A commitment's kind is a tick, a number, a note or a total* — a kind never changes, and
      ADR-1030 carries why
- [x] 2.4 *A range is a lowest and a highest, and the lowest is not above the highest* — the
      refuse-rather-than-adjust argument is dropped; the MUST NOT sentences stay as they are
- [x] 2.5 *A target is a number above zero* — see 2.4
- [x] 2.6 *A commitment is due exactly when its schedule is due, on and after the day it is kept from*
- [x] 2.7 *A commitment is not due before the day it is kept from* — keep the sentence that the floor
      does not shift the schedule's own start date or phase
- [x] 2.8 *A roster holds the commitments a person keeps, in the order they were taken on* — keep
      that neither a changed nor a superseded commitment gains a fifth part and no link is held
- [x] 2.9 *A roster refuses a commitment it already holds* — **not split**, over budget; keep that a
      target is compared by whole value exactly as a range is
- [x] 2.10 *A roster stops keeping a commitment, on the day it was kept until*
- [x] 2.11 *A roster answers which commitments it had not stopped keeping on a calendar date*
- [x] 2.12 *A roster store keeps a roster at a place, across the app being closed and opened again* —
...
- [x] 2.13 *A roster store reads a roster kept before a commitment carried a kind* — ADR-1031 says
      which forms are read; keep every sentence about what each form reads back as
- [x] 2.14 *A roster store that cannot be read is refused rather than emptied* — the argument is
      ADR-1035's now; keep all three MUST NOTs and the four things that could not be a roster
- [x] 2.15 *A commitments screen lists the commitments its roster keeps, in the order they were taken
      on* — holds one of the two knowingly-false scenario titles; its bookkeeping paragraph goes
- [x] 2.16 *A commitments screen lists what has been stopped, beside what it keeps* — the "would
      double the structure of the screen" argument is dropped; the flat-list MUST NOT stays
- [x] 2.17 *A commitments screen defines a commitment from a name, a rhythm and the day it is kept
...
- [x] 2.18 *A commitments screen refuses a name that says nothing, and a rhythm due on no day*
- [x] 2.19 *A commitments screen refuses a rhythm number the calendar will not take* — 28% xref;
      2.41 defers to this for how two refusals are told apart, and must not restate it
- [x] 2.20 *A commitments screen tells a commitment it already keeps apart from a roster it could not
      write* — keep that the already-holds-it refusal covers all three states, not only kept
- [x] 2.21 *A commitments screen asks you to confirm before it stops keeping a commitment* — holds
      the second knowingly-false scenario title; its bookkeeping paragraph goes
- [x] 2.22 *A commitments screen takes a stopped commitment up again in one tap* — the category comes
      back with it, which 2.40 relies on
- [x] 2.23 *A commitments screen keeps its roster at the place a day screen keeps its, and reads it
      again when the app is shown* — the `Application Support` argument is ADR-1017's
- [x] 2.24 *A commitments screen that cannot read its roster lists nothing and changes nothing*
- [x] 2.25 *A commitments screen holds the change it refused and why, one at a time* — the seven
...
- [x] 2.26 *What a commitments screen holds about a refused change lasts until the app is shown again
      or a change is kept* — one act, one outcome stays as a rule; its argument is ADR-1038's
- [x] 2.27 *A roster removes a commitment it holds, and never lets it go* — ADR-1035 carries why
- [x] 2.28 *A commitments screen removes a commitment only when its name is typed back*
- [x] 2.29 *A roster moves a commitment among the ones it keeps*
- [x] 2.30 *A commitments screen moves a commitment among the ones it keeps* — defers to 2.29 in one
      clause rather than restating what a move is
- [x] 2.31 *A roster puts a commitment under a category* — keep the same-word rule in both halves;
      only its identity half has a scenario. ADR-1038 carries why a category is the roster's
- [x] 2.32 *A roster reads the commitments it is keeping in groups, one per category*
- [x] 2.33 *A roster moves a group among the groups it is keeping* — keep the "whatever state that
      last commitment is in" qualifier; ADR-1044 records it as previously mis-implemented
- [x] 2.34 *A commitments screen moves a group among the groups it draws* — a refused group move
      names the category, not one of its commitments
- [x] 2.35 *A roster answers the earliest day anything it holds has been kept from* — the aggregate
...
- [x] 2.36 *A roster changes a commitment it holds for another, in the place it holds it* — keep that
      a roster holds no records and carries none over
- [x] 2.37 *A roster supersedes a commitment it is keeping with another, from a day* — keep the
      supported-range ends and that no link is held to the commitment that replaces it
- [x] 2.38 *A commitments screen says what a commitment it is asked to change is made of* — the
...
- [x] 2.39 *A commitments screen changes a commitment on either of its lists* — **splits in two**,
...
- [x] 2.40 *A commitments screen offers the categories in use* — the offering-as-safety argument
      becomes ADR-1038's; the MUST NOT fold case and MUST NOT match loosely stay
- [x] 2.41 *A commitments screen refuses a range that is not a range, and a target that is not a
...
- [x] 3.1 `openspec validate condense-commitment-spec --strict` exits 0.
...
- [x] 3.3 `pnpm run check:budgets` reports no requirement over 150 words except the thirty-one
...
- [x] 3.4 `pnpm run verify` passes.
- [x] 3.5 `swift test` in `src/DayByDayKit` passes and reports the same number of tests as it does on
      `main` — read off both runs, never derived — because this Story touches no test file.
- [x] 3.6 **The archive handover — `implementer` ticks this in its last commit before the archive**,
