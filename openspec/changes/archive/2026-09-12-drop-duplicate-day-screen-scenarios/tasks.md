...
- [x] 3.1 Restore `a commitment ticked on the date has a row that says it is kept` in `DayViewTests.swift` from `origin/main`, unchanged — withdrawn by grill item 20
...
- [x] 3.14 Restore `a commit on a row for a day that has not arrived is told nothing on the row` in `DayScreenTests.swift` from `origin/main`, unchanged — withdrawn by grill item 17
- [x] 3.15 Restore `a commit on a note row for a day that has not arrived is told nothing on the row` in `DayScreenTests.swift` from `origin/main`, unchanged — withdrawn by grill item 17
...
- [x] 4.3 *A day view holds the commitments due on a date, each with whether it is kept* — the scenario of 3.1 is carried in place
...
- [x] 4.11 *A day screen tells nothing on a row where there was no change to refuse* — the two scenarios of 3.14 and 3.15 are carried in place
...
- [x] 5.3 None of the twenty-six dropped titles is found as an exact test name under `src/`, each
...
- [x] 5.5 `swift test` in `src/DayByDayKit` passes and reports twenty-six fewer tests than on `main`,
