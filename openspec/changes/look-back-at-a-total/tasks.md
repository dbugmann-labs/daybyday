...
- [ ] 10.2 `git diff --stat origin/main` lists only this change folder, `CONTEXT.md`, `docs/adr/1045-*`, `docs/open-questions.md` (the G4-time amendment `design.md` § *The graph walk grows* names), the two kit sources, the kit test file and `LookBackView.swift`
...
- [ ] W.6 **The handover** — the four pictures are posted to the PR with `pnpm run walk -- --post-only <pr>`, never `gh pr comment --attach`, before hand-back, and this box is ticked on that comment's URL
