...
- [x] W.2 A total with sums on past days whose target was then changed from 120 to 100 through the edit sheet, and a sum added today: the rule stands at 120 then 100 with no riser, "120" labelled where it steps and "100" at its newest end, today's point judged against 100
...
- [x] W.5 **The handover** — the four pictures are posted to the PR with `pnpm run walk -- --post-only <pr>`, never `gh pr comment --attach`, before hand-back, and this box is ticked on that comment's URL: https://github.com/dbugmann-labs/daybyday/pull/311#issuecomment-5792703245
