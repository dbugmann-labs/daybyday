...
- [ ] 16.1 `openspec validate copy-on-every-change --strict` exits 0 and `pnpm run check:scenarios` exits 0
- [ ] 16.2 `git diff --stat origin/main` lists only this change folder, `CONTEXT.md`, `docs/adr/`, the kit sources and tests, `CommitmentsView.swift` and `ContentView.swift`
- [ ] 16.3 `pnpm run check:budgets` warns about this folder only for the one MODIFIED requirement `day-screen` carries whole
