...
- [ ] 7.5 `ContentView.swift`'s Return on the one-off entry drops focus and closes the keyboard once the add is kept, as the checkmark does, and keeps focus with the typed text on a refused add — as `design.md` § *The shell* says (grill answer 25) — and `pnpm run verify` passes and the app target builds again after it

## 8. The records

- [ ] 8.1 Confirm `CONTEXT.md` § *Day view*, § *One-off* and § *One-off entry* still describe what shipped; a sentence that turns out wrong is a **stop and a G4 question**, never an edit slipped in
