...
- [ ] 1.2 Confirm the five facts the shape rests on, before writing any test, and stop and report if
...
- [ ] 1.3 Re-read this box before § 2 and again before § 4. `commitment` and `day-screen` are the
...
- [ ] 2.1 `a roster holding no commitments answers no earliest day anything it holds is kept from` —
  adds the member.
- [ ] 2.2 `a roster answers the earliest day among the commitments it holds` — and the same three
...
- [ ] 2.3 `a roster counts a commitment it has stopped keeping in the earliest day anything it holds is kept from`
  — this is what forces the walk over `entries`. An implementation over `commitments` passes 2.1 and
  2.2 and fails here.
- [ ] 2.4 `a roster counts a commitment it has removed in the earliest day anything it holds is kept from`
  — ADR-1035: a roster never lets a commitment go, and this is that rule read over one more question.
- [ ] 2.5 `the earliest day anything a roster holds is kept from falls when a commitment kept from an earlier day is taken on`
- [ ] 2.6 `a roster answers the day a commitment is kept from and not a day it is due` — if
  `Schedule` or `isDue(on:)` appears anywhere in the implementation, stop and report it.
- [ ] 2.7 `a roster answers the first supported date where a commitment it holds is kept from it`
...
- [ ] 3.1 `a day screen's day picker opens on the day it is showing` — adds `Reach` and
  `dayPickerReach`, both answered off state the screen already holds.
- [ ] 3.2 `a day screen's day picker reaches back to the earliest day anything on its roster is kept from`
  — asks `Roster.earliestKeptFrom` from § 2 and does not recompute it.
- [ ] 3.3 `a day screen's day picker reaches back past a commitment its roster has stopped keeping`
- [ ] 3.4 `a day screen's day picker reaches back past a commitment its roster has removed`
- [ ] 3.5 `a day screen's day picker reaches back to the day it is showing where that is the earlier of the two`
  — the clamp. An implementation that used the roster's answer unconditionally passes everything
  above and fails here.
- [ ] 3.6 `a day screen that cannot read its roster reaches back to the today it was handed` — the
  fallback, which is the `nil` from § 2 and no state of its own.
- [ ] 3.7 `a day screen that takes on the commitments it was handed reaches back to the earliest of those`
  — a first launch is **not** the fallback case: day one is written before the screen answers
  anything (ADR-1027).
- [ ] 3.8 `a day screen whose roster stops being readable goes on showing its day and reaches back to it`
...
- [ ] 3.9 `a day screen that cannot read its record says the reach of its day picker like any other`
  — if `recordState` or `recordStore` appears in `dayPickerReach`, stop.
- [ ] 3.10 `a day screen's day picker reaches back to the first supported date and opens on the last`

### 3b. A day screen shows a day picked on its day picker — eleven scenarios

- [ ] 3.11 `a day screen shows a day picked between the earliest day its picker reaches and the day it was showing`
  — adds `showDay(_:)`.
- [ ] 3.12 `a day screen shows a day picked after the today it was handed` — forward is open, to the
  last supported date.
- [ ] 3.13 `a day screen shows the earliest day its day picker reaches when that day is picked` — the
  bound includes its own day. A `>` where a `>=` belongs fails here and nowhere else.
- [ ] 3.14 `a day screen is left exactly as it was by a day picked earlier than its day picker reaches`
...
- [ ] 3.15 `a day screen picking the day it is already showing changes nothing`
- [ ] 3.16 `a day screen picking a day draws the commitments its roster had not stopped keeping on that day`
- [ ] 3.17 `picking a day on a day screen does not read its roster or its record again`
- [ ] 3.18 `picking a day on a day screen does not change the today it was handed`
- [ ] 3.19 `a day screen stops telling what it was telling on a row when a picked day changes the day it is showing`
...
- [ ] 3.20 `a day screen goes on telling what it was telling on a row when a picked day is earlier than its day picker reaches`
  — the refused pick clears nothing. An implementation that clears the notice before the guard
  passes 3.19 and fails this.
- [ ] 3.21 `a day screen offers the way back to today once a day other than that today is picked` —
...
- [ ] 4.1 Add a `DatePicker` with `displayedComponents: [.date]` to the day-title row
...
- [ ] 4.2 Leave the chevrons, the *Today* button, the swipe recognizer and every row untouched. If
...
- [ ] 4.3 Walk it on the phone: `pnpm run phone`. Confirm five things and report anything else you
...
- [ ] 5.1 Confirm `ContentView.swift` compares no two days of its own. **`Reach.opensOn` makes the
...
- [ ] 5.2 Confirm `CONTEXT.md` § *Reach* and § *Day picker* still describe what shipped — the
...
- [ ] 5.3 Confirm no ADR was written or amended by this branch:
...
- [ ] 5.5 Leave `docs/backlog.md` alone. B-040 is answered by this Story, and moving a want to
...
- [ ] 6.1 `cd src/DayByDayKit && swift test` — every test green, and the count is **961**: 933 at
...
- [ ] 6.2 `openspec validate add-day-picker --strict` exits 0, and `openspec validate --all --strict
