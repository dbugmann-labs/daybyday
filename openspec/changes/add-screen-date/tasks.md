...
- [x] 1.1 Add `Sources/DayByDayKit/DayTitle.swift`, internal: the seven weekday names and the twelve
...
- [x] 1.2 Add `public func title(asOf today: CalendarDate) -> String` to `DayView` and
...
- [x] 2.1 `a day view says its day as a weekday, a day of the month, a month and a year` — Monday
  31 August 2026 asked as of Thursday 3 September 2026, expecting `"Monday 31 August 2026"`. The
  first tick of the seam, and the two-digit day of the month. Ran red on the `fatalError`, as
  predicted.
- [x] 2.2 `a day view of the day it is asked as of says Today before the date` — Thursday
  3 September 2026 asked as of itself, expecting `"Today · Thursday 3 September 2026"`. The middle
  dot is U+00B7 with a space on each side. Ran red: the day asked as of was ignored, as predicted.
- [x] 2.3 `a day view of a day before the one it is asked as of says the date and not Today` —
  Wednesday 2 September 2026 as of Thursday 3 September 2026. The negative control for 2.2: without
  it, "Today · " could be unconditional. Passed immediately — 2.2's equality check already covers
  it — rather than running red; recorded per this section's instruction rather than assumed.
- [x] 2.4 `a day view of a day after the one it is asked as of says the date and not Today` — Friday
  4 September 2026 as of Thursday 3 September 2026. The other side of the same control, and the one
  that fails an implementation comparing "not later than" rather than "equal". Passed immediately,
  which confirms the implementation compares equality rather than "not later than".
- [x] 2.5 `a day of the month below ten is said without a leading zero` — Tuesday 1 September 2026
  as of Thursday 3 September 2026, expecting `"Tuesday 1 September 2026"` and not `"01"`. Passed
  immediately — `date.day` is an `Int`, and `Int` string interpolation is never padded.
- [x] 2.6 `every weekday is said by its own name` — the seven day views from Monday 31 August 2026
  to Sunday 6 September 2026, each asked as of Thursday 1 January 2026, which is none of them, so no
  title says "Today". Seven full strings asserted. Passed immediately — the weekday table written
  at 1.1 already held all seven names correctly.
- [x] 2.7 `every month is said by its own name` — the fifteenth of each month of 2026, each asked as
  of Thursday 1 January 2026. Twelve full strings asserted; this is the test that catches a typo in
  the month table. Passed immediately — the month table written at 1.1 already held all twelve
  names correctly.
- [x] 2.8 `a day view says its day in the first supported year and in the last` — Saturday 1 January
  1583 as of Monday 3 January 1583, and Friday 31 December 9999 as of Monday 27 December 9999. Both
  ends of what `CalendarDate` accepts. Passed immediately — no edge in the implementation treats
  the range boundaries specially.
- [x] 2.9 `a day view says the leap day of a leap year` — Tuesday 29 February 2028 as of Monday
  28 February 2028. Passed immediately — the implementation reads `date.day` and `date.month`
  directly and treats no month specially.
- [x] 2.10 `a day view holding no rows says its day just the same` — a day view of no commitments at
  all on Wednesday 2 September 2026. Assert both that it holds no rows and that it says its day:
  the title is read off the date, not off the rows. Passed immediately — `title(asOf:)` never
  reads `rows`.
- [x] 2.11 `a day screen says the day it is showing` — opened as of Thursday 3 September 2026 at a
  place where nothing has been kept, expecting `"Today · Thursday 3 September 2026"`. Ran red on
  the second `fatalError`, as predicted.
- [x] 2.12 `a day screen says the day it was handed rather than the day it really is` — Monday
  3 January 1583 and Monday 27 December 9999. Any implementation that reaches for `Date()` fails
  both halves. Passed immediately — 2.11's implementation already delegates to `today` without
  reading a clock.
- [x] 2.13 `a day screen says the day its own day view says, asked as of the day it was handed` —
  assert `screen.title` equals `screen.dayView.title(asOf:)` for the day it was opened on. The
  assertion that the screen delegates rather than formats a second time. Passed immediately, which
  is direct confirmation of the delegation.
- [x] 2.14 `a day screen shown again on a later day says that day` — opened Monday 31 August 2026,
  then `shown(asOf:)` Tuesday 1 September 2026. Fails an implementation that computed the title once
  at opening. Passed immediately — `title` is a computed property, per `design.md` § *The seam*,
  never stored.
- [x] 2.15 `a day screen that cannot read its record still says the day` — a place holding a run of
  bytes that is not a record, built the way `DayScreenTests` already builds one. Assert both
  `recordState` and the title: ADR-1021 applied to a second question. Passed immediately — `title`
  reads `dayView` and `today`, neither of which is affected by `recordState`.
- [x] 2.16 `a day screen says the same day after a tick is made on it` — tick the screen's own row
  and assert the title is unchanged, which is #91's "a day screen does not change day when a tick is
  made on it" seen from the title. Passed immediately — `tick(_:)` never touches `today`, and
...
- [x] 3.1 In `src/DayByDay/DayByDay/ContentView.swift`, draw `Text(screen.title)` above the rows in
...
- [x] 3.2 Build and run it: `xcodebuild -project src/DayByDay/DayByDay.xcodeproj -scheme DayByDay
  ...
  Built with `xcodebuild -project src/DayByDay/DayByDay.xcodeproj -scheme DayByDay -destination
  'platform=iOS Simulator,name=iPhone 17' build` — **BUILD SUCCEEDED**. Booted, installed and
  launched on the iPhone 17 simulator (`59430515-851B-41A8-9BE7-76B5F59DE053`) via `xcrun simctl
  boot`/`install`/`launch`, on 2026-09-03. A screenshot taken with `xcrun simctl io screenshot`
  shows the string **"Today · Thursday 3 September 2026"** drawn as the first row of the list,
  above "Run", "Reading", "Supplements and habits" (ticked), "Journaling" (ticked) and "Water
  plants" — one line, no wrapping, plainly readable at a glance on the device's own clock reading
  of the day this Story was worked (Thursday 3 September 2026).
