...
that date — whether it is kept, and, where its commitment's kind is a number or a note, the number
or the note that day holds. Two rows SHALL be the same row when all three agree, and SHALL be
different when any one of them differs.
...
reason it grew by the number rather than for a new one.
...
kind, so this follows from what a history says rather than being a rule this capability adds.
...
kept, and **what it offers** — a tick, a number entry or a note entry, according to its commitment's
kind. It MUST NOT give back the commitment itself, the schedule underneath it, the day it is kept
from, the date the row is for, **the number that day holds**, or **the note that day holds**: what a
reader is given is what a screen draws and what a tap makes, and nothing else has been asked for.

Neither the number nor the note is among them, though a row now holds one or the other. Each is
given out only inside the entry a row offers, so that a screen drawing a row has nothing to draw it
with: a row for a day holding 70.5 and a row for a day holding "Ran 8k." each say what a ticked row
says and no more. That is a shape rather than a rule someone has to keep, which is why what the day
holds is part of what a row *is* and no part of what a row *gives back* — and it matters more for a
note than for a number, since a note is the one record long enough that drawing it in a list would
change what the list is for.
...
back was asked for, it MUST NOT tell which of writing a note and taking one back was asked for, and
...
**A note names no cause at all, and the two stay two.** A note commitment declares no bound, and
every text that says something is a note, so there is nothing this screen can refuse a committed
note for but the place refusing to take it — and a text that says nothing is not a refusal but the
take-back. A note SHALL therefore be told exactly as a refused tick is: on the row, naming nothing.
Nothing about a note's length, its script, its line breaks or its characters SHALL be named as a
cause, because no such refusal exists to name and inventing a message for one would teach a person
...
that lands, a tick taken back, a number entered and a number taken back, a note written and a note
taken back all count: what is being told is that the place would not take your change, and any one
of them landing is proof to the contrary. One rule rather than six, because a person cannot act on
the difference. A change that does not reach the place SHALL NOT end it — a second refusal moves it
rather than ending it, and a tap that changes nothing at all changes this nothing either.
...
it, so a note is only ever one of the ends above or one of the refusals the place itself makes.
...
offers no tick, no number entry and no note entry, asked as of the today the screen was handed, so
there is no change to refuse and nothing was asked of the place. What is told on a row means a
change did not reach the place, and here there was no change. The honest answer to a day that has
not arrived is a row that does not invite the tap at all, and that is not this capability's answer
here — for the third kind of row now, and the price is paid a third time rather than half-fixed.
...
A commit on a **row that offers no entry at all for any other reason** SHALL be the fourth: a tick
row and a total row were never asked for anything to be committed and have nothing to refuse. What
is committed on one keeps nothing and says nothing, exactly as a tick made on a number row or a note
row does — a row answers for the kind its commitment declares and stays silent about every other.

**A note row is no longer among them**, and that is what this change moves here. A note row offers a
note entry, so a commit on one is read, kept and answered for exactly as a commit on a number row
is; what it never does is name a cause, since a note has no refusal of its own to name. A commit
that is *read* and lands, or is read and refused by the place, is the requirement above and not this
one; this requirement covers only a commit that was never read at all.
...
- **WHEN** a day screen is opened as of Monday 31 August 2026, at a place where nothing has been
  kept, of a commitment named "Water" of the total kind with a target of 120, on a schedule listing
  Monday, Wednesday and Saturday, kept from 1 January 2026, and "Ran 8k." is committed on its one
  row
- **THEN** the day screen tells nothing on any row
- **AND** its day view still says the commitment is not kept on that date
- **AND** committing "Ran 8k." on a row of a day screen alike in every way but of a commitment named
  "Gym" of the tick kind tells nothing on any row either
...
A row SHALL offer at most one of a tick, a number entry and a note entry, and never two of them. A
commitment's kind says what its days take and it says one thing: a row whose commitment's kind is a
tick offers the tick alone, one whose kind is a number offers the number entry alone, one whose kind
is a note offers the note entry alone, and one whose kind is a total offers none of the three, on
every day, until there is a record for what those days take. Which of them it offers is therefore
the only thing a row says about its kind — a row of any kind says its name, the rhythm it runs on
and whether the day was kept in exactly the same way.
